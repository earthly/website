...
Originally I thought I would be able to use the tview package I'd used in my last article for this project. It has a grid system that I figured I could take advantage of to act as my game world, but that didn't turn out to be the case. I don't really remember the exact technical reason I ended up moving away from Tview, but basically I wanted to be able to access and update the exact x and y coordinates of elements in the terminal, and as far as I could tell, that wasn't easilly possible in Tview.
...
To get started, we create a tcell Screen. This represents the physical terminal screen. Tcell is set up to detect what type of terminal you are using, so you shouldn't need to configure it after creation.
...
If your not used to thinking about your screen as a graph with an X axis and a Y axis, you'll need to start. You may be used to graphs that start with (0,0) in the lower left hand corner. But for Screens put (0,0) in the upper left. So when we run the code from the last section, we get the text to appear at the top of the screen.


Ok, but if we run this code, it appears as though nothing happens. That's because the we need to call `Show()` on our screen. This will display everything we set up in SetContent.
...
Now if you run this you'll see "Hi!" written in your terminal. The loop is keeping our programming running so it doesn't exit. It's also repeatedly writing "Hi!" to the screen. We can't tell because it's just overlapping itself, but "Hi!" is getting written over and over. This is important to note for the next section.
...
One of the most important features of any video game is the ability to get upset at it and then quit. We can't quit our game yet because we need to set up some code that can read user input and react. For now, you can "quit" your program by closing your terminal window and opening a new one.
...
Ok so we took a long time to get here but a lot of these concepts, the screen is a graph, updating during our event loop, redrawing and reacting to input, will be used over and over as we develop the game from here. In the next post, we'll use channels to funnel user input to the game. 

Movement
    How to make the make a traveling zero

Creating a game that you can only rage quit. User input. The ability to rage quit

Below is all the code for part one which just allows a character to travel off the screen. It also allows the user to quit.

There's no point in creating a video game unless the player has the ability to mumble an obsennity under their breath (or scream it into their microphone) and then rage quit your game. So let's set that up first.
...
func Run(s tcell.Screen) {
	...
	for {
		...
		s.SetContent(n, 10, '0', nil, defStyle)
