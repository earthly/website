...
Java with the JVM really set the bar for cross platform development. No more did you developer environment need to match you production environment. With the JVM you could compile things down to JARs and run it anywhere with a Java Virtual Machine installed. 
...
The state of the art though appears to be Zig, which can easily cross compile not just Zig programs but also anything that would normally be build with Clang or GCC.
...
There is also things that peopel were certain would change computing and become new table stakes but yet haven't. 
 * REPLs
 * Image based computing (a la small talk)
 *  Jupter notebooks (Python)
 * Integrated Data (Wolfram / MatLab)
 * 
