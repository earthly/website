...
---
**We're [Earthly](https://earthly.dev/). We make building software simpler and therefore faster using containerization. Even when you are building pong games to play in the terminal, Earthly can help. [Check it out](/).**
...
### Moving the Ball
<!--sgpt-->
In this tutorial, we modified our "Hi!" to move across the screen by adding X and Y speed variables to our Ball struct in Go. We updated the ball's position with an Update function. We added a Ball to our Game struct, and also adjusted the for loop in our `Run` function to use the ball, which can now move in any direction on the screen. The ball will disappear when it hits the edge, so we added a CheckEdges function to make it bounce back. The ball's speed changes from positive to negative to switch between adding and subtracting. Now we have a bouncing ball! 

If you're building Golang projects like this one, you might want to give [Earthly](https://www.earthly.dev/) a try for efficient and consistent builds. It could streamline your development process and ensure that your builds are reproducible across different environments.

You can personalize the bouncing ball, or wait for the next post when we'll add more features. The current code is available [here](https://github.com/jalletto/pongo).
