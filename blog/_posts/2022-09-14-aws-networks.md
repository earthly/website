...
Ok, so a Region is just a part of the world, and an availability zone is just sections of that region where the data centers are. That brings us to the [VPC]() 
...
If you don't like clicking on links to things, here's my best attempt at an explication.

IP addresses are four numbers, each one is called an octet because it represents an 8 bit value. 8 bits means each number can range in value from 0 - 255. So 0.0.0.0 to 255.255.255.255

Ip addresses actually represent 2 pieces of information. The address for the device, but also the address for the network the device is on. So for example 
...
Give your instance a name and then under `Application and OS Images` you can select an OS to to use on your instance. I'll be using Ubuntu Server 22.04 for this tutorial.
