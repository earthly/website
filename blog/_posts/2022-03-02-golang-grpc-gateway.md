...
Second I'll build a REST service, using the same proto file but that uses the same implementation as the existing gRPC service. Assuming you have a shared backing database, you could use this solution to scale the REST end-point separately from the gRPC end-point.
...
Ok, I have a REST service now, but what are the end-points? What type of requests can I make and what type of response should I expect? In all cases I checked, in this version of the gRPC gateway, the expected request format and the responses given look like just a straight conversion to JSON.
...
There we go, a single service that supports gRPC and REST requests. Let's test it with my gRPC client:
