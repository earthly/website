...
A small team of developers, at a SASS company with a couple of hundred developers has a problem. They own a couple of services that do some data loading and transforming. And the services are under increased load because a new customer (Customer-A) is generating many times the amount of data that most customers do.
...
So Tim studies all the existing services, and starts mapping things out. It turns out that across all the services queuing and work in progress is handled in every possible way. It's hard to get a global view of everything, but mainly, work is being tracked using SQS, database tables, or Kafka.
