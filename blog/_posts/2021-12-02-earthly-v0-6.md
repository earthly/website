...
What's more, Earthly is able to create references across directories and across repositories. Meaning that Earthly can be a bridge between different programming language ecosystems. If you need to put together artifacts or container images from JS (NPM, Yarn etc), Java (Maven, Gradle etc), Python (Pip, Pipenv etc), Ruby (Bundler etc), or even PHP (Composer etc), you can use Earthly to put together these otherwise disconnected worlds.
