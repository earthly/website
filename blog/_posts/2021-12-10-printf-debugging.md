...
author: Adam
 ...
 - debugging
---
The Visual Studio debugger is fantastic. When I was a C# developer, I used it all the time. A typical pattern was putting a breakpoint on a failing assert in a unit test and then just poking around. I would look at all values in the current scope and see if anything looked wrong. Then I could drag the execution point up to earlier in the unit test and walk through the code.  

I would do this pretty quickly without really thinking much about the problem at hand, and I could usually find my way to where the problem was. Of course, it might take me some time to find the problem, but often I could find it iteratively and a bit mindlessly.

When I was younger, finding problems like this seemed like a lazy superpower. I would see other devs writing out assumptions about code or talking through how it worked in an attempt to find pinpoint an issue in their understanding, but I could just debug through the code base until I found the problem.[^1]

The strange thing is that many incredible developers don't use debuggers at all. And it's not like debuggers are a new technology that haven't made their way out into the world yet. Richard Stallman released GDB in 1986. That was 35 years ago! And yet many developers don't use debuggers or even warn against their over-usage:
...
The secret to printf debugging isn't the printfs. I can use those just as mindlessly as a debugger. The secret is thinking, thinking through the execution of your program, and deepening your understanding of how your program works. Doing so gives you a better ability to simulate program execution in your head.

Debugging problems with careful thought and the odd printf statement is like deriving the proofs from first principles yourself. It's hard -- more challenging than the alternative -- and sometimes it may be beyond your ability, but you'll be a better software engineer for it.
