...
While it’s impossible to know in advance which files an arbitrary command will access during execution, Ucacher is based on the observation that given a past execution, we can check if its initial state is maintained for the actual running environment to conclude that a new execution would produce the same results. 
...
2. Then, for each potential entry, verify whether the hashes of the input files from that execution match the current filesystem state. If they match, then we have a cache hit. 
