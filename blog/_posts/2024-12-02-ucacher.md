...
The approach to identifying matching initial conditions differs from a traditional one-hop cache lookup. 
Given it’s impossible to know in advance which files an arbitrary command will access during execution, Ucacher employs a two-phase lookup process as follows:
1. Retrieve all cached entries associated with a specific combination of command, environment, operating system, and branch.
2. For each cached entry, verify whether the hashes of the input files from that execution match the current filesystem state. If they match, then we have a cache hit. 

The insight is that checking only the hashes of that particular set of input files is enough to guarantee that if they match, then the execution would be identical and hence the output files can be cached. 
