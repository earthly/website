...
*Introducing [ucacher](https://github.com/earthly/setup-ucacher): a powerful new tool from Earthly that simplifies file caching and command skipping in GitHub Actions. Designed for effortless adoption and better precision, Ucacher eliminates manual errors and supports a broad range of use cases.*
...
In this post we'll describe the current standard to perform caching and skipping in Github Actions to later on introduce Ucacher, a tool we've been recently working on that automates such tasks at the command level, eliminating the need for manual configuration and achieving higher precision.
