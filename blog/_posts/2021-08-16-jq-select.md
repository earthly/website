...
`jq` is a filter in the UNIX command line sense. You pipe (`|`) a JSON document to it and it filters it and outputs it to standard out. We could easily use this feature to chain together jq invocations like this:
``` bash
echo '{"title":"JQ Select"}' | jq '.title' | jq 'length'
9
```
This is a verbose, though simple, way to determine the length of a string in a JSON document. We can use this same idea to combine various `jq` built-in functions with the features we have seen so far. But there is an easier way. You can use pipes inside of `jq` and conceptually they work just like shell pipes. 
...
Whenever I need to extract data or transform a JSON document, I try to do it first without looking anything up. If memory fails me, sometimes [jqterm](https://jqterm.com/), which has auto-completion, is helpful. Often though, I still end up needing to look something up, but science has shown that [repeated retrieval yields retention](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3983480/). So over time, my retention should improve.

#### 3. Use It

If you don't use a tool, you will never master it. So when I have a task that can solved using `jq`, then it is what I use. At least for the next little while, even if there is an easier way to do it. Whether its exploring a REST API or looking at `docker inspect` results, JSON is everywhere. 

#### 4. Learn More

Lastly, to deepen my knowledge I'm learning about recursive descent, and declaring variables, and defining functions and the other features listed in the [manual](https://stedolan.github.io/jq/manual/#Advancedfeatures). These things rarely come up, but at this point I'm hooked on this tool. 

Doing all this isn't necessary, but if you follow me in some of these steps, then I think using `jq` will become second nature. 
