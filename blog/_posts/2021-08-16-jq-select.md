...
By default `jq` pretty-prints all its output. This mean that it's output is not character for character identical to the input. I'm going to ignore the pretty printing. It's not the hard part of the tool.
</div>

## Using JQ to Select Elements

Let's start with the GitHub repository API and use jq to select out the data we need. The default data returned looks like this:
``` bash
...
The JSON documents returned by the GitHub API are quite large. Therefore, for ease of explanation, I will omit non-relevant details.
</div>
...
If you need raw strings, the `-r` option in `jq` gives you that.
...
The `-j` option (for join) can combine together output.
...
Not everything emitted by `jq` is proper JSON. When I extracted titles from the `jq` issues api, my result was not a valid JSON array. It was a new line delimited list:
``` json
"Question about license."
"visibility of wiki pages"
...
...
2340
```
This uses pipes, which I'll will talk about shortly and causes my object selectors to be run per element.
...
Most of the things I've used so far are filters:
 ...
 * `[2:4]` is a filter that, given an array, returns elements 2 through 4.
 ...
 You can join these filters together using with `jq`'s built-in functions. You do this using a pipe (`|`) just like in a UNIX shell.
...
Let's say I want to grab all the issues that are labeled, skipping all the drive-by fix-my-problem issues. It's not possible to do this with the GitHub API unless you specify all the possible labels in your query. Never fear though, we can easily do this query on the command line by filtering our results with `jq`.
...
You can think of `map(...)` as a shorthand for `[ .[] | ...  ]` - unwrap this array, apply the filter and then rewrap the results back into an array. 
...
This is using 3 object indexes, 2 maps, 2 pipes, a length function, and a `select` predicate, but if you've followed along, this should all make sense. It's all just composing together filters until you get the result you need. 
...
This has only been the basics of `jq`. The `jq` query language is a full programming language and you can do lots of exciting things with it. Like define you own functions and even [finding primes](https://rosettacode.org/wiki/Sieve_of_Eratosthenes#jq):
...
```
 ...
 However, simple stuff like selecting elements, filtering values by key, or by value is often all you need. 
