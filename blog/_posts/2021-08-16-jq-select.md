...
Let's start learning how to select elements using the GitHub api for `jq`. The data returned looks like this:
``` bash
...
```
And you can use `[]` to get all the elements in array. This is useful when you want to apply furthering filtering to elements of the array. 
...
Not everything emitted by `jq` is valid JSON. When I extracted titles from the `jq` issues api, my result was not a valid JSON array. It was a new line delimited list:
``` json
"Question about license."
"visibility of wiki pages"
...
```
 ...
 In order to put a collection of results produced by a filter into a JSON array, you can just wrap it in `[ ... ]` where `...` is a filter.
...
The GitHub issues API has a lot of details I don't care about. I'd like to select multiple fields from the document returned and leave the rest behind.
...
2340
```
This causes my object selectors to be run per array element and get me closer to the result I want.
...
But this still isn't the JSON document I need. In order to get these values into a proper JSON document the object constructor `{ ... }` is needed. The object construction syntax lets you build up a JSON object using `jq` filters for both keys and values.
...
To put the elements you've selected back into a JSON document you can wrap them in an array constructor `[ .. ]` or a object constructor `{ ... }`.
 ...
 * `[2:4]` is a filter that give an array returns elements 2 through 4.
 ...
 You can join these filters together using with jq's built functions using a pipe `|` just like in a unix shell.
...
All this means that sorting my labels is simple. I can just change `.labels` to `.labels | sort`:
...
The `jq` issues list has a lot of non-issues: people can't figure out how to do something and open an issue. ** Attention Internet: Please don't do this.** GitHub issues aren't the place to post your random homework problem. 

Lets say I want to grab all the issues that are labeled, skipping all the drive-by issues. It's possible to query for a specific label using issues api, but just getting all issues that have any labels at all is a bit more tricky especially if the set of possible labels is vast. Never fear though, we can easily do this query on the command line by filtering our results with `jq`.
...
This is uses 3 object indexes, 2 maps, 2 pipes, a length function, and a select predicate but if you've followed along this should all make sense. It's all just composing together filters and functions until you get the result you need. 
