...
The `jq` issues list has many non-issues: people can't figure out how to do something and open an issue[^4]. Let's say I want to grab all the issues that are labeled. This would let me skip all the drive-by fix-my-problem issues. Unfortunately, it's impossible to do this with the GitHub API unless you specify all the possible labels in your query. Never fear though, we can easily do this query on the command line by filtering our results with `jq`.
...
[^4]: Please don't do this. GitHub issues aren't the place to post your random struggles with understanding a tool.