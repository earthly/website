...
`jq` is a lightweight, command-line JSON processor. I install it with brew (`brew install jq`), but its a single portable executable so it's easy to get into whatever environment you are working on. 
...
And you can use `[]` to get all the elements in the array. This is handy when you want to further filter the array contents (which I'll be doing shortly).

For Example, here is how I would get the titles of the jq issues returned by my API request:
...
...
```
In fact, whenever you ask `jq` to return an unwrapped collection of elements, it prints them each on a new line. You can see this by explicitly giving it a more than one element to return.
...
2
```
The way we resolve this is the same way we would turn the text `1,2` into an array into the text for an array in JavaScript: by wrapping it in the array constructor `[ ... ]`.
...
```
Similarly, to put a collection of results produced by using an array index into a JSON array, you just wrap it in `[ ... ]`.
...
This uses pipes, which I'll talk about shortly, and runs my object selectors (`.title` and `.number`) on each array element.
...
But this still isn't the JSON document I need. To get these values into a proper JSON object I need the object constructor `{ ... }`. 

## Putting Elements Into an Object Using `jq`

Let's look at a simple examples before showing how the object constructor can be used with the GitHub API. 
...
To wrap those values into the shape I need I can just replace the values with the array indexes that return them:
``` json
{
  "first-name":.[0],
  "last-name":.[2]
...
```
This is the same way you would create an object in a JSON document, except you can use object and array queries you've built up as the values.
...
3
```
These built-ins can be combined with the selectors we've built up so far. That's what I'm going to show you next.
...
`jq` is a filter, in the UNIX sense: You pipe (`|`) a JSON document to it, and it filters and outputs it to standard out. However, the `jq` language itself is also made up of filters.
