...
K3s can also be deployed alongside Docker or Docker Desktop using the community-developed [k3d project](https://k3d.io). K3d is a lightweight wrapper that runs K3s inside a container. It lets you run multiple K3s clusters on a single host and administer them using familiar Docker tools. You can install k3d with the following command:
