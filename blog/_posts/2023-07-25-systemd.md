...
These unit files live in several different places. It's helpful to become familiar
...
We could do a whole other blog post on journalctl, but I do want to mention it here briefly as it can be one of the great advantages of using systemd. When systemd manages a service unit, it automatically captures its standard output and standard error streams and stores them in the journal. This means that any log messages or output generated by the service will be recorded and made accessible through journalctl.

We can check logs for a specific unit, in this case hello-world, with `journalctl -u hello-world`. This will give you all the logs, which can be overwhelming to page through. More often you'll probably want logs from a certain time frame when debugging. For that you can use the `--since` and `--until` flags.

~~~{.bash caption=">_"}

journalctl -u hello-world --since "2023-07-23 12:00:00" --until "2023-07-23 18:00:00"
~~~

## Conclusion

I really had a hard time narrowing down just what to cover in this article. Systemd is a far larger and more complicated subject than I had anticipated before deciding to write this. What I've tried to do is provide a hands on introduction to some of systemd's inner workings and hopefully recreate the lessons and bits of knowledge I learned that really helped me start to get a feel for systemd.
...
- Reading up on targets in more depth and to help you grasp the concept of grouping units and how they can be used to achieve specific system states or goals. Targets are powerful tools for managing the system's behavior during boot-up or when switching between different system modes.
...
Lastly, since I know you were wondering what happens if you run `sudo kill 1`, I can tell you that on an Ubuntu EC2 machine, it does not explode or cause a fire at Amazon, which is a bummer. 

When I ran it, at first it seemed like nothing happened. The system kept running and I even stayed connected via ssh. The only change I noticed was that the `/sbin/init` got replaced by `/lib/systemd/systemd --system --deserialize 25` when I checked `htop`.

<div class="wide">

![`/sbin/init` replaced by `/lib/systemd/systemd --system --deserialize 25`]({{site.images}}{{page.slug}}/4.png)

</div>

So I assume there's code somewhere that knows to immediately restart systemd, it just calls it directly rather than accessing it through the symlink. No clue if this is a Linux, Amazon, or Ubuntu thing. If anybody has the answer, please let me know, but also, probably just don't ever do this.
...
So whatever failsafe is in place seems to restart systemd with and restore it to a previous saved state, which I guess is indicated by that number 25.
