...
So I assume there's code somewhere that knows to restart systemd immediately if it get's killed, it just calls it directly rather than accessing it through the symlink. No clue if this is a systemd or Ubuntu thing. If anybody has the answer, please let me know, but also, probably just don't ever do this.
...
So whatever fail-safe is in place seems to restart systemd and restore it to a previous saved state, which I guess is indicated by that number 25. I'm going to keep digging, but that's probably for another blog post.
