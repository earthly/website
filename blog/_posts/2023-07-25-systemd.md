...
When I ran `sudo kill 1`, at first it seemed like nothing happened again. The system kept running, and I even stayed connected via ssh. The only change I noticed was that PID 1 had changed from `/sbin/init` to `/lib/systemd/systemd --system --deserialize 25`.
